...
R3D_MeshData R3D_CreateMeshData(int vertexCount, int indexCount)
{
    ...
    if (vertexCount <= 0) {
        R3D_TRACELOG(LOG_ERROR, "R3D: Invalid vertex count for mesh creation");
        return meshData;
    }

    meshData.vertices = RL_CALLOC(vertexCount, sizeof(*meshData.vertices));
    if (meshData.vertices == NULL) {
        R3D_TRACELOG(LOG_ERROR, "R3D: Failed to allocate memory for mesh vertices");
        return meshData;
    }
    ...
    if (indexCount > 0) {
        ...
        if (meshData.indices == NULL) {
            R3D_TRACELOG(LOG_ERROR, "R3D: Failed to allocate memory for mesh indices");
...
R3D_MeshData R3D_DuplicateMeshData(R3D_MeshData meshData)
{
    ...
    if (meshData.vertices == NULL) {
        R3D_TRACELOG(LOG_ERROR, "R3D: Cannot duplicate null mesh data");
        return duplicate;
    }
...
R3D_MeshData R3D_MergeMeshData(R3D_MeshData a, R3D_MeshData b)
{
    ...
    if (a.vertices == NULL || b.vertices == NULL) {
        R3D_TRACELOG(LOG_ERROR, "R3D: Cannot merge null mesh data");
        return merged;
    }
...
void R3D_GenMeshDataTangents(R3D_MeshData* meshData)
{
    ...
    if (bitangents == NULL) {
        R3D_TRACELOG(LOG_ERROR, "R3D: Failed to allocate memory for tangent calculation");
        return;
    }
